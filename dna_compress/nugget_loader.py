...
NUGGET_LATENT_MODES = ("dense", "continuous_bottleneck", "flatten_bottleneck")
...
class NuggetAutoencoder(torch.nn.Module):
    def __init__(
        ...
        *,
        ...
        self.flatten_input_dim = self.flatten_max_nuggets * self.code_dim
        if self.latent_mode in {"continuous_bottleneck", "flatten_bottleneck"}:
            self.encoder_to_code = torch.nn.Linear(self.hidden_dim, self.code_dim)
            self.code_to_decoder = torch.nn.Linear(self.code_dim, self.hidden_dim)
            if self.bottleneck_layer_norm:
                self.code_layer_norm = torch.nn.LayerNorm(self.code_dim)
    ...
    def _quantize_nugget_encoding(self, encoding: torch.Tensor, mask: torch.Tensor):
        if self.latent_mode == "dense":
            return SimpleNamespace(encoding=encoding, bottleneck_encoding=None)
        if self.latent_mode == "continuous_bottleneck":
            ...
            return SimpleNamespace(encoding=decoder_encoding, bottleneck_encoding=projected)
        if self.encoder_to_code is None or self.code_to_decoder is None:
        ...
        if projected.shape[1] < self.flatten_max_nuggets:
            padded = projected.new_zeros(projected.shape[0], self.flatten_max_nuggets, self.code_dim)
        ...
        if self.decoder_input_layer_norm is not None:
            decoder_encoding = self.decoder_input_layer_norm(decoder_encoding)
        return SimpleNamespace(encoding=decoder_encoding, bottleneck_encoding=bottleneck)
    ...
    def quantize_nuggets(self, nuggets):
        ...
        return SimpleNamespace(
            ...
            bottleneck_encoding=quantized.bottleneck_encoding,
        )
    ...
    def decode_quantized_nuggets(
        ...
        *,
        ...
        output.decoder_loss = output.loss
        return output
...
def build_nugget_model(model_config: ModelConfig, tokenizer_spec: NuggetTokenizerSpec) -> tuple[NuggetAutoencoder, NuggetBackboneSpec]:
    ...
    return (
        NuggetAutoencoder(
            ...
            code_dim=model_config.nugget_code_dim,
